...
svmLFit <- train(Y~., data = data_train_final,
                 ...
                 trControl = control)
svmLFit
aSVMl<-svmLFit$results
plot(aSVMl$C,aSVMl$RMSE)
svmLFit$bestTune

predicaoSVMl<-predict(svmLFit,select(data_test_final,-Y_holdout))
rmseSVMl<-rmse(data_test_final$Y_holdout,predicaoSVMl)
rmseSVMl
